...
const MovieCredits = () => {
    ...
    if (movie_id) {
        const { data: movie, error: isErr } = useMoviesDetail({ id: movie_id });
        const { data: credits, error } = useMovieCredits({ id: movie_id });
        ...
        if (error || isErr) {
            return (
                <div className="pt-20 min-h-dvh container mx-auto">
                    ...
                    <CardContent className="flex items-center gap-4">
                        ...
                        <div className="space-y-3">
                            ...
                            <Link to={`${loc.pathname.replace("/cast", "")}`}>
                                <Button variant={"ghost"} className="text-lg text-gray-300">
                                    <ArrowLeft /> Back to the main
                                </Button>
                            </Link>
                        </div>
                    </CardContent>
                ...
                <div className="grid grid-cols-2">
                    <div className="flex flex-col gap-3">
                        <h1 className="text-xl font-semibold">
                            Cast{" "}
                            <span className="text-gray-400">({credits?.cast.length})</span>
                        </h1>
                                ...
                                <img
                                    ...
                                    src={
                                        item.profile_path != null
                                            ? `${IMAGE_URL}/original${item.profile_path}`
                                            : "../../../public/blank.png"
                                    }
                                    alt={item.name}
                                />
                    ...
                    <div className="flex flex-col gap-3">
                        <h1 className="text-xl font-semibold">
                            Crew{" "}
                            <span className="text-gray-400">({credits?.crew.length})</span>
                        </h1>
